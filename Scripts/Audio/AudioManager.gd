...
@export_group("Ciclico sounds")
@export var hook_sound:AudioStream
...
@export var memory_collected:AudioStream
@export var flame_gained:AudioStream
@export var enemy_explosion:AudioStream

@onready var intro_song_player:AudioStreamPlayer = %IntroSongPlayer
@onready var battle_song_player:AudioStreamPlayer = %BattleSongPlayer
...
@onready var move_shoot_player: AudioStreamPlayer = %MoveShootPlayer
...
func _play_intro_music():
	intro_song_player.play()

func _play_ambience_ia():
...
func _play_ambience(sound:AudioStream):
...
func _fade_out_intro_player():
	var pre_tween_volume = intro_song_player.get_volume_db()
	var fade_out_tween = get_tree().create_tween()
	fade_out_tween.tween_property(intro_song_player, "volume_db", -48.0, 3.0)
	await fade_out_tween.finished
	intro_song_player.stop()
	intro_song_player.set_volume_db(pre_tween_volume)
	
func _fade_out_battle_player(duration:float = 3.0):
	var pre_tween_volume = battle_song_player.get_volume_db()
	var fade_out_tween = get_tree().create_tween()
	fade_out_tween.tween_property(battle_song_player, "volume_db", -48.0, duration)
	await fade_out_tween.finished
	battle_song_player.stop()
	battle_song_player.set_volume_db(pre_tween_volume)

func _fade_in_battle_player():
	var pre_tween_volume = battle_song_player.get_volume_db()
	battle_song_player.volume_db = -60.0
	var fade_in_tween = get_tree().create_tween()
	battle_song_player.play()
	fade_in_tween.tween_property(battle_song_player, "volume_db", pre_tween_volume, 5.0)
	await fade_in_tween.finished

func _fade_in_intro_player():
	var pre_tween_volume = intro_song_player.get_volume_db()
	intro_song_player.volume_db = -60.0
	var fade_in_tween = get_tree().create_tween()
	intro_song_player.play()
	fade_in_tween.tween_property(intro_song_player, "volume_db", pre_tween_volume, 5.0)
	await fade_in_tween.finished
...
func _play_hook_sound():
	_play_game_sfx_1D(hook_sound, -3.0, 1.0, true, move_shoot_player)
